...
class BasicMerger(object):
    ...
    def __init__(
        ...
        csv_file1_path: str,
        csv_file2_path: str,
        minimum_address_matching_ratio: float = 0.50,
        ...
        logging.debug(
            f"New merger: paths: "
            f"{self.csv_file1_path} and {self.csv_file2_path}. "
            f"Min address matching r.: {minimum_address_matching_ratio}. "
            f"Uniq. address? {unique_addresses}"
        )

    def merge(
        self,
        output_path: str,
        extra_csv1_fields: dict = None,
        extra_csv2_fields: dict = None
    ):
        """
        ...
        for row1 in csv1:
            ...
            while row2_index < len(csv2):
                ...
                # Two addresses are equal when their normalized addresses match
                # with a matching ration equals to
                # self.minimum_address_matching_ratio
                # Note we pass the addres set to the matcher,
                # not the actual addresses
                # to avoid compute the normalized addresses again and again
                addresses_match, row2_address_score = self.addresses_match(
                    row1["normalized_addresses"], row2["normalized_addresses"]
                )
                ...
                if addresses_match:
                    row3 = {
                        ...
                        "var1": row1["variable1"],
                        "var2": row2["variable2"],
                        "ratio": BasicMerger.ratio_from_rows(row1, row2)
                    }
                    ...
                    csv3.append(row3)
                    logging.debug(
                        f"Success! {row1} was merged to {row2} in {row3}"
                    )

                    # If addresses don't have more than one occurence,
                    # when the first matching is found, end matching process
                    # for this address
                ...
                if not row1_has_a_match:
                    logging.error(
                        f"Error. {row1['id_store']};'"
                        f"{row1['address']}' couldn't be merged. "
                        f"Closest address is "
                        f"{closest_row['address'] if closest_row else ''}"
                        f"({closest_score})"
                    )

        # Output the resultant CSV file
        BasicMerger.write_output_csv(
            output_path,
            csv3,
            list(extra_csv1_fields.values()),
            list(extra_csv2_fields.values())
        )
        ...
        merged_address_percentage =\
            100.0 * merged_address_count / csv1_address_count
        logging.info(
            f"Merge of addresses "
            f"{merged_address_count} "
            f"of "
            f" {csv1_address_count} "
            f"({merged_address_percentage} %) completed in {output_path} "
            f"Spent time: {elapsed_time} seconds"
        )

    def addresses_match(self, addresses1: set, addresses2: set) -> tuple:
        """
        Inform if the normalized address sets
        of two addresses are enough similar.
        """
        same_addresses_count = len(addresses1 & addresses2)
        address_matching_ratio = \
            same_addresses_count / len(addresses1 | addresses2)
        return (
            address_matching_ratio >= self.minimum_address_matching_ratio,
            address_matching_ratio
        )

    @staticmethod
    def ratio_from_rows(row1, row2):
        """
        Compute the ratio of variable1 and variable 2
        from the two rows.
        If variable 2 is zero, ratio should be infinity,
        so it it set to null
        """
    ...
    def read_csv(csv_file_path: str):
        ...
        with open(csv_file_path, 'r') as csv_file:
            ...
            for row in csv_file_reader:
                row["normalized_addresses"] =\
                    set(BasicMerger.address_expander(row["address"]))
                rows.append(row)
        return rows
    ...
    def write_output_csv(
        csv_file_path: str,
        rows: [],
        extra_csv1_fields: None,
        extra_csv2_fields: None
    ):
        """
        ...
        with open(csv_file_path, 'w') as csv_file:
            csv3_fields = (
                ["id", "var1", "var2", "ratio"] +
                extra_csv1_fields + extra_csv2_fields
            )
            csv_file_writer = csv.DictWriter(
                csv_file, fieldnames=csv3_fields, delimiter=';', quotechar='"'
            )
    ...
    def address_expander(address: str) -> list:
        ...
        logging.debug(
            f"{address} has {len(expaned_addresses)} "
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename=f'{this_file_path}/log/basic_merger.log',
        format=f'%(asctime)s %(levelname)s: [{execution_uuid}] %(message)s',
        level=logging.INFO
    )
    if debug:
